...
static
int basisChange(double M[], double *xyz, int dim) {
	double point[3], det, invM[3*3];
	if ( dim == 2 ) {
		point[0] = xyz[0]; point[1] = xyz[1];
		det      = M[0] * M[3] - M[2] * M[1];
		if ( fabs(det) < DTOL ){
			printf(" BASIS IS LINEARLY DEPENDENT! ");
			return EGADS_GEOMERR;
		}
		invM[0] =  M[3]/det;
		invM[1] = -M[1]/det;
		invM[2] = -M[2]/det;
		invM[3] =  M[0]/det;
		xyz [0] = invM[0] * point[0] + invM[1] * point[1];
		xyz [1] = invM[2] * point[0] + invM[3] * point[1];
	} else {
		point[0] = xyz[0]; point[1] = xyz[1]; point[2] = xyz[2];
		det  = M[0] * M[4] * M[8] + M[1] * M[5] * M[6] + M[2] * M[3] * M[7];
		det -= M[2] * M[4] * M[6] + M[5] * M[7] * M[0] + M[8] * M[3] * M[1];
		if ( fabs(det) < DTOL ){
			printf(" BASIS IS LINEARLY DEPENDENT! ");
			return EGADS_GEOMERR;
		}
		...
		invM[3] = (M[6] * M[5] - M[3] * M[8])/det;
		invM[4] = (M[0] * M[8] - M[2] * M[6])/det;
		invM[5] = (M[3] * M[2] - M[5] * M[0])/det;

		invM[6] = (M[3] * M[7] - M[4] * M[6])/det;
		invM[7] = (M[6] * M[1] - M[0] * M[7])/det;
		invM[8] = (M[4] * M[0] - M[3] * M[1])/det;
		xyz [0] = invM[0] * point[0] + invM[1] * point[1] + invM[2] * point[2];
		xyz [1] = invM[3] * point[0] + invM[4] * point[1] + invM[5] * point[2];
		xyz [2] = invM[6] * point[0] + invM[7] * point[1] + invM[8] * point[2];
	}
	return EGADS_SUCCESS;
}
...
computePenalty(quadMap *qm,  int starID, int qID, double *penalty, double *angle) {
	...
	double curvature[8];
	int i, stat, validArea, q, v, tooSmall = 0, invalid = 0, vv;
	...
	/* stat = EG_curvature(qm -> face, &qm -> uvs[2*v], curvature);
  if ( fabs(curvature[0] * curvature[3]) < EPS) {
    if ( fabs(qm ->vCurvs[8*v] * qm ->vCurvs[8*v + 3]) > MINKCHANGE)
      invalid = 0;
  }
  else if (curvature[0] * curvature[3] * qm ->vCurvs[8*v] * qm ->vCurvs[8*v + 3] < 0.0)
    invalid = 1;
  for ( i = 0 ; i < 3; ++i) dev[i] = qm ->xyzs[3*v + i ] - qm ->oriXYZ[3*v + i ];
  normOri = sqrt(dev[0] * dev[0] + dev[1] * dev[1] + dev[2] * dev[2]);
  if ( normOri > qm -> vLengths[2*v] ) invalid = 1;
 ...
 //   printf(" INVALID %d  NORM 0  distorted\n",invalid);
  ...
  if ( norm[1] > qm -> vLengths[2*v] || norm[1] < qm -> vLengths[2*v + 1]) {
	...
	*penalty = zerr * exp(zerr *scale/M_PI);
	...
	//else if ( invalid == 1) *penalty = zerr + exp(1 + norm[0] + norm[1]);
#ifdef DEBUG_AREA
	printf("\n APPLiYING PENALTY TO QUAD %d:: erfc  %lf  penalty %lf  DOT %lf angle %lf  NEG AREA %d\n",qID, zerr, *penalty, dot, alpha*180.0/M_PI, validArea);
...
quad_algebraic_area(quadMap *qm, int qID, int vID,  int *validArea, double *intAngle ) {
	int i, vPos, v, qV[4], k, tri, piv[4], stat;
	double vA[18], vB[18], vC[18], cross[4], dotCross[4], area[2*4], z, zerr, alpha, dot, norm[2], theta[2*4], evalVert[18], normal[4],uv_dir[2];
#ifdef DEBUG_AREA
	...
	printf(" NORMAL TO SURFACE AT VERTEX %lf  %lf  %lf\n",normal[0], normal[1], normal[2]);
	printf(" PLANE SURFACE %d\n",PLANESURFACE);
	if(PLANESURFACE) {
		printf(" RESET PROPER NORMAL\n");
		normal[0] *= -1.0; normal[1] *= -1.0; normal[2] *= -1.0;
	}
	normal[3] = normal[0] *normal[0] + normal[1] *normal[1] + normal[2] *normal[2];
	normal[3] = sqrt(normal[3]);
	printf(" du %lf %lf %lf   dv %lf %lf %lf\n", evalVert[3], evalVert[4], evalVert[5], evalVert[6], evalVert[7], evalVert[8]);
	printf(" NORMAL TO SURFACE AT VERTEX %lf  %lf  %lf\n",normal[0], normal[1], normal[2]);
	for ( k = 0 ; k < 4; ++k) { // Check additional area to ensure that the quad is correct
		printf(" AREA CENTRED AT VERTEX %d\n", qV[(k  + vPos          )%4] + 1);
		for ( tri = 0 ; tri < 2; ++ tri) {
			piv[0]  = qV[(k  + vPos          )%4];
			piv[1]  = qV[(k  + vPos + 1 + tri)%4];
			piv[2]  = qV[(k  + vPos + 2 + tri)%4];
			printf(" CENTRE %d  ANGLE 012  %d %d %d\n",vPos, piv[0], piv[1], piv[2]);
			uv_dir[0] = qm -> uvs[2*piv[0]    ] + (qm -> uvs[2*piv[1]    ] - qm -> uvs[2*piv[0]    ])/10.;
			uv_dir[1] = qm -> uvs[2*piv[0] + 1] + (qm -> uvs[2*piv[1] + 1] - qm -> uvs[2*piv[0] + 1])/10.;
			stat = EG_evaluate(qm -> face, uv_dir, vB);
			uv_dir[0] = qm -> uvs[2*piv[0]    ] + (qm -> uvs[2*piv[2]    ] - qm -> uvs[2*piv[0]    ])/10.;
			uv_dir[1] = qm -> uvs[2*piv[0] + 1] + (qm -> uvs[2*piv[2] + 1] - qm -> uvs[2*piv[0] + 1])/10.;
			stat = EG_evaluate(qm -> face, uv_dir, vC);
			norm[0] = 0.0; norm[1] = 0.0; dot = 0.0;
			for ( i = 0 ; i < 3; ++i){
				vA[i]    = qm -> xyzs[3*piv[0] + i ];
				dot     += (vB[i] - vA[i])*(vC[i] - vA[i]);
				norm[0] += (vB[i] - vA[i])*(vB[i] - vA[i]);
				norm[1] += (vC[i] - vA[i])*(vC[i] - vA[i]);
			}
			norm[0] = sqrt(norm[0]); norm[1] = sqrt(norm[1]);
			if ( norm[0] < 1.E-7 || norm[1] < 1.E-8)  theta[2*k + tri] = 0.0;
			else {
				printf(" ANGLE A %lf %lf  %lf AB %lf %lf  %lf  AC %lf %lf %lf\n",vA[0], vA[1], vA[2], vB[0],vB[1],vB[2],vC[0],vC[1],vC[2]);
				dot /= (norm[0] * norm[1]);
				printf(" DOT %lf  NORMS %lf %lf\n",dot,norm[0], norm[1]);
				if ( fabs(dot - 1.0) < DTOL )      theta[2*k + tri] = 0.0;
				else if ( fabs(dot + 1.0) < DTOL ) theta[2*k + tri] = M_PI;
				else                               theta[2*k + tri] = acos(dot);
			}
			// BUILD A FLAT ELEMENT AND CHECK IF AREA IS VALID
			for ( i = 0 ; i < 3; ++i){
				vA[i] = qm -> xyzs[3*piv[0] + i ];
				vB[i] = qm -> xyzs[3*piv[1] + i ];
				vC[i] = qm -> xyzs[3*piv[2] + i ];
			}
			area[2*k + tri] = triang_cross_area(vA, vB, vC, cross);
			printf(" CROSS PRODUCT %d %d %d = %lf , %lf, %lf\n",(k  + vPos)%4,(k  + vPos + tri + 1)%4,(k  + vPos + tri +2)%4,cross[0], cross[1], cross[2] );
			cross[3]        = cross[0] *cross[0] + cross[1] *cross[1] + cross[2] *cross[2];
			cross[3]        = sqrt(cross[3]);

			dotCross[k]     = cross[0] * normal[0]/(cross[3]*normal[3]) + cross[1] * normal[1]/(cross[3]*normal[3]) + cross[2] * normal[2]/(cross[3]*normal[3]);
			if ( dotCross[k] < -0.1 ) theta[2*k + tri] = 2.0 * M_PI - theta[2*k + tri];  // we have anti-clockwise orientation. Postive area implies reversed.
#ifdef DEBUG_AREA
			printf("\n K %d  TRI %d\n AREA BETWEEN VERTEX %d %d %d \n", k,tri,piv[0] + 1,piv[1] + 1,piv[2] + 1);
			//printf(" TRIANG AREA FOR A= %lf %lf B= %lf %lf C= %lf %lf\n", vA[0], vA[1], vB[0], vB[1], vC[0], vC[1] );
			printf(" AREA TRI= %lf NORMS %lf   %lf  ANGLE  %lf  DOTCROASS %lf\n",area[2*k+tri], norm[0], norm[1], theta[2*k + tri], dotCross[k]);
#endif
		}
	...
	*intAngle  = M_PI*0.5;
	...
	for ( k = 0 ; k < 4; ++k) {
		//if (fabs(theta[2*k] + theta[2*k +1]  + 1.0 ) < EPS) *validArea = -1;
		alpha = theta[2*k] + theta[2*k+1];
		if ( alpha > 2.0*M_PI) alpha -= 2.0*M_PI;
		if ( k != 2) {
			if ( alpha >= M_PI) *validArea = -1;
			if (fabs(*intAngle - M_PI*0.5) < fabs(alpha - M_PI*0.5)) *intAngle = alpha;
		}
		printf(" ANGLES %lf   AND AREAS %lf  ANGLE WITH SURFACE NORMAL  %lf\n",alpha, area[2*k] + area[2*k + 1], dotCross[k]);
	}
#ifdef DEBUG_AREA
...
#if MULTIOPTI
	nMove = 0 ;
	printf(" STAT = %d WE MADE IT HERE WITH VERTEX %d %d %d\n",stat, v[0], v[1], v[2]);
	if ( stat != EGADS_SUCCESS) exit(1);
	for ( i = 0 ; i < 4; ++i)
		if ( qm -> vType[v[i] -1] == -1) movingVerts[nMove++] = v[i];
...
double NLOPT_objEqualAnglesTess(unsigned n, const double *uvs, double *grad, void *inQM)
{
	...
	for ( i = 0 ; i < qm->nS; ++i) {
		...
		for (j = 0 ; j < qm->star[i] ->nQ; ++j) {
			...
			fStar += fQuad + penalty;
			fprintf(filOpti,"%d %d\t %lf %lf %lf\t %lf %lf %lf %lf \n",qm -> star[i] -> quads[j],  qm -> star[i]->verts[0] ,theta, fabs(thetaOpt - theta),thetaOpt, fQuad,fStar,fTot,  penalty);
		}
	...
	if (OPTCOUNT%NOUT == 0 ) {
		...
		for ( i = 0 ; i < qm->nS; ++i) print_starFile(qm -> star[i], qm);
	}
	return fTot;
}
...
static int optimize_angles(quadMap *qm, int nV, int *vID)
{
	...
	for (j = 0; j < nV; ++j) {
		...
		for ( i = 0 ; i < qm->star[j] -> nQ; ++i) {
			stat = quad_algebraic_area(qm, qm->star[j] ->quads[i], qm->star[j] ->verts[0], &area, &angle);
...
int main(int argc, char *argv[])
{
	...
	stat = 0;
	mod_xyzs = (double *) EG_alloc(3*len*sizeof(double));
	if ( mod_xyzs == NULL ) return EGADS_MALLOC;
	for (i = 0; i < len; i++) {
		mod_xyzs[3*i]     = xyzs[3*i  ];
		mod_xyzs[3*i + 1] = xyzs[3*i + 1 ];
		mod_xyzs[3*i + 2] = xyzs[3*i + 2 ];
	}
