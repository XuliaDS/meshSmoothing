...
static int
EG_edgeSplit(quadMap *qm, vStar *star, int performSplitting) {
  ...
  if ( qm -> valence[ star -> verts[0] -1][0] < 5 ) return EGADS_SUCCESS;
  if ( qm -> remQuads[0] == 0 ) {
...
EG_mergeVertices(quadMap *qm, int qC, int poly[], int idCol, int collapseToCentre) {
  int vC, vC2,vID, stat, i, e, q, v, vCommon, j, adjQ, k, updatedQuad, vVAL, aux, auxVal[100], addValence;
...
EG_vertexCollapse(quadMap *qm, vStar *star) {
  ...
  if (stat != EGADS_SUCCESS) goto cleanup;
  vC = poly[2*(coll[cp][0])];
  EG_buildStar(qm, &star, poly[2*(coll[cp][1])]);
#ifdef DEBUG
  printf(" IN DOUBLET WE HAVE COLLAPSED VERTEX %d to %d AND REMOVED QUAD %d\n",poly[2*(coll[cp][0])],poly[2*(coll[cp][1])], qC + 1);
#endif
  for ( j = 0 ; j < star -> nV; ++j) {
    auxID =  - 1;
    if ( star -> verts[j] > 0) {
...
EG_edgeSwap(quadMap *qm, vStar *star)
{
  ...
  if ( swapped ) {
#ifdef DEBUG
    printf("WE have swapped edge %d %d for %d %d\n", poly[0], poly[1*2], poly[2*2], poly[3*2]);
#endif
#if MULTIOPTI
    nMove = 0 ;
    for ( i = 0 ; i < 6; ++i)
...
static int EG_swapAndSplit(quadMap *qm, vStar *star) {
  ...
  if (validSwap) {
    ...
    if ( EG_edgeSplit(qm, star, 0)) {
      ...
      if (stat != EGADS_SUCCESS)
        return resetQm(qm, locIdx, locAdj, locVtype, locVal, uv0);
      stat = EG_edgeSplit(qm, star, 1);
      if ( stat != EGADS_SUCCESS) return resetQm(qm, locIdx, locAdj, locVtype, locVal, uv0);
    }
  }
  return EGADS_SUCCESS;
}
...
{
  int          nSweeps = 1000, activity, itQ, nPeaks, i, j, k, count, kOK, id0, auxID, auxQ, nseg, newUV, stat, nVars, quadID, prevQuad;
  ...
  while ( k < nSweeps && activity) {
    ...
    for (i = 0 ; i < len; ++i) {
      if (qm -> vType[i] == -1) {
        stat = EG_buildStar(qm, &star, i + 1);
        printf(" SPLITTING IN UV SWEEP %d  CALLING FOR STAR AROUND %d\n",k, i+1);
        count = TOTSPLITS;
        stat = EG_edgeSplit(qm, star,1);
        if ( TOTSPLITS > count) {
          activity = 1;
    ...
    for (i = 0 ; i < len; ++i) {
          if (qm -> vType[i] == -1) {
            stat = EG_buildStar(qm, &star, i + 1);
            printf(" SWAP AND SPLIT UV SWEEP %d  CALLING FOR STAR AROUND %d\n",k, i+1);
            count = TOTSPLITS;
    ...
    fprintf(fop," ---------------------------------------------\n");
    fclose(fop);
    printf(" TOTAL SWAPS IN THIS ROUND   %d, COLLAPSES %d  SPLITS %d STAT = %d\n", TOTSWAPS, TOTCOLLAPSES,TOTSPLITS, stat);
...
static int optimize_angles(quadMap *qm, int nV, int *vID)
{
  ...
  if ( stat2 != EGADS_SUCCESS) {
    for (j = 0; j < nV; ++j) print_starFile(qm ->star[j],qm, CONVC, OPTCOUNT );
    for (j = 0; j < nV; ++j) {
