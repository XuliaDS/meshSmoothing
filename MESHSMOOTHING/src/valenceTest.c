...
computePenalty(quadMap *qm,  int starID, int qID, double *penalty, double *angles) {
  ...
  double curvature[8];
  int i, j, id0, stat, validArea, q, v, movingVert, tooSmall = 0, invalid = 0, vv;
  ...
  printf(" COMPUTING PENALTY FUNCTION::: VERTEX %d in quad %d VALID AREA %d  ANGLE %lf ---> LOCAL ANGLE %lf\n",v,q,validArea, alpha, *angles);
  if ( stat != EGADS_SUCCESS) {
  ...
  for ( i = 1; i < 4; ++i) {
    ...
    if ( i != 2) {
      ...
      printf(" MOVING VERT %d  COMP %lf  TO   %lf\t",movingVert, fabs(M_PI*0.5 - angles[i] ) , fabs(M_PI*0.5 - alpha ));
      if( (movingVert == 1) && ( fabs( (2.0 * M_PI)/(double)qm ->valence[vv - 1][0] - angles[i] ) > fabs( 2.0*M_PI/(double)qm->valence[qm ->star[starID] -> verts[0] - 1][0] - angles[0] ) ) )
        alpha = angles[i];
    }
    printf(" ALPHA %lf\n",alpha);
  }
...
quad_algebraic_area(quadMap *qm, int qID, int vID,  int *validArea, double *quadAngles) {
  ...
  double vTri[18*3], evalVert[18], naux, cross[4], dotCross[4], area[2*4], z, zerr, alpha, dot, norm[2], theta[2*4], normal[4], vNormal[4], uv_dir[2], uv_eps[2],  vPro[8], vEps[8];
#ifdef DEBUG_AREA
  ...
  for ( k = 0 ; k < 4; ++k) { // Check additional area to ensure that the quad is correct
    ...
    for ( tri = 0 ; tri < 2; ++ tri) {
      ...
      for ( i = 0 ; i < 3; ++i) {
        vPro[i]     = vTri[18 + i] ;//- vTri[i];
        vPro[4 + i] = vTri[36 + i] ;//- vTri[i];
      }
      area[2*k + tri] = triang_cross_area(&vTri[0], &vPro[0], &vPro[4], cross);
      //area[2*k + tri] = triang_cross_area(&vTri[0], &vTri[18], &vTri[18*2], cross);
      for ( i = 0 ; i < 3; ++i) {
        vPro[i]     -= vTri[i];
        vPro[4 + i] -= vTri[i];
        //vPro[    i] = vTri[18   + i ] - vTri[i];
        //vPro[4 + i] = vTri[18*2 + i ] - vTri[i];
      }
      EG_unitVector(&vPro[0], &vPro[3]);
      EG_unitVector(&vPro[4], &vPro[7]);
      ...
      dotCross[k] = cross[0] * normal[0] + cross[1] * normal[1] + cross[2] * normal[2];
  ...
  *validArea = 1;
  ...
  for ( k = 0 ; k < 4; ++k) {
    ...
    if ( quadAngles[k] >= M_PI*0.99) *validArea = -1;
...
double NLOPT_objEqualAnglesTess(unsigned n, const double *uvs, double *grad, void *inQM)
{
  ...
  if (OPTCOUNT%NOUT == 0 ) {
    printf(" SUM STARS FUNCTION AT COUNT %d  IS  %lf\n",OPTCOUNT, fTot);
    for ( i = 0 ; i < qm->nS; ++i) {
...
int main(int argc, char *argv[])
{
  ...
  stat = 0;
  mod_xyzs = (double *) EG_alloc(3*bodydata[0].plen*sizeof(double));
  ...
  char buffer[32];
  snprintf(buffer, sizeof(char) * 32, "ORIGINALUVverts.txt");
  printf(" WRITING ON FILE %s\n",buffer);
  ...
  for ( i = 0 ; i < bodydata[0].plen; ++i) {
    printf(" I % d\n",i);
    EG_evaluate(bodydata[0].faces[FRONTFACE], &bodydata[0].uvs[2*i], eval);
    fprintf(fout, "%d  %d\t %lf %lf  \t",i + 1, ptype[i], bodydata[0].uvs[2*i], bodydata[0].uvs[2*i + 1]);
    for ( j = 0 ; j < 9; ++j) {
      fprintf(fout, "%lf\t",eval[j]);
    }
    fprintf(fout,"\n");
