...
typedef struct {
  ...
  int      nQ, nS, totVerts,setIndex;
  ego      face;
  double  *xyzs, *oriXYZ, *uvs,  *qArea, *vCurvs;
  vStar   **star;
} quadMap;
...
static int EG_createQuadMap(bodyData *bodydata)
{
  ...
  int           len, ntri, nquad, vID0, vID1;
  double        dist, surfArea, perimetre, aux, uvbox[2*2], uvLims[2*4],  verts[18*4], daux, hmin, hmax;
  ...
  for ( f = 0 ; f < bodydata->nfaces; ++f) {
    ...
    bodydata->qm[f] -> qArea    = (double*) EG_alloc(6*nquad  *sizeof(double));
    ...
    if (bodydata->qm[f]-> quadIdx  == NULL  ||bodydata->qm[f] ->  quadAdj == NULL ||bodydata->qm[f] ->  xyzs     == NULL ||
        ...
        bodydata->qm[f] -> qArea == NULL ||bodydata->qm[f] ->  vCurvs  == NULL ||bodydata->qm[f] ->  remQuads == NULL ||
    ...
    stat = EG_getRange(bodydata->qm[f] -> face, uvbox, &iper);
    uvLims[0] = uvbox[0]; uvLims[1] = uvbox[2];
    uvLims[3] = uvbox[1]; uvLims[4] = uvbox[2];
    uvLims[5] = uvbox[1]; uvLims[6] = uvbox[3];
    uvLims[7] = uvbox[0]; uvLims[7] = uvbox[3];
    for ( j = 0 ; j < 4; ++j)
      stat = EG_evaluate(bodydata->qm[f] -> face, &uvLims[2*j], &verts[18*j]);
    perimetre = 0.0;
    for ( j = 0 ; j < 4; ++j) {
      dist = 0.0;
      for ( i  = 0 ; i < 3; ++i) {
        aux   = (verts[18*( (j+1)%4 ) + i] - verts[18*j + i]) * (verts[18*( (j+1)%4 ) + i ] - verts[18*j + i]);
        dist += aux;
      }
      perimetre += sqrt(dist);
    }
    printf(" TOTAL AREA %lf  PERIMETRE %lf\n",surfArea,perimetre);
    surfArea  /= (double)nquad;
    for ( nseg = j = 0 ; j < len; ++j) {
      if (bodydata->qm[f] -> vType[j] != -1) nseg++;
    }
    perimetre /= (2.0*(double)nseg);
    printf(" ELEMENT AREA %lf  SEGS %d  .> PERIMETRE %lf\n",surfArea,nseg, perimetre);
    for ( j = 0 ; j < nquad; ++j) {
      for ( k = 0 ; k < 4; ++k) {
      ...
      bodydata->qm[f] -> qArea[6*j    ] =        surfArea;
      bodydata->qm[f] -> qArea[6*j + 1] = 0.75 * surfArea;
      bodydata->qm[f] -> qArea[6*j + 2] = 1.25 * surfArea;
      bodydata->qm[f] -> qArea[6*j + 3] =        perimetre;
      bodydata->qm[f] -> qArea[6*j + 4] = 0.75 * perimetre;
      bodydata->qm[f] -> qArea[6*j + 5] = 1.25 * perimetre;
    }
  }
...
computePenalty(quadMap *qm,  int starID, int qID, double *penalty, double *angles, double *quadArea) {
  double dot, z, zerr, alpha, dev[3], nAux,   minmax[2], scale, curv[8], projV[3*2],  proj[3], norms[2], normal[3], thetaK, kSUM, hTheta[2], areaParallelogram;
  ...
  for ( j = 0 ; j < 2; ++j) {
    ...
    EG_unitVector (&curv[5], &nAux);
    printf(" PROJECT %d %d onto %dplane  %lf %lf %lf\n",sID +1, link +1,sID + 1, proj[0], proj[1], proj[2]);
    dot = proj[0] * curv[5] + proj[1] * curv[6] + proj[2] * curv[7];
    if ( fabs(dot - 1.0 ) < EPS ) thetaK = M_PI;
    else thetaK = acos(dot);
    printf(" ANGLE WITH CURVATURE %lf \n", thetaK);
    kSUM = KMIN*cos(thetaK)*cos(thetaK) + KMAX*sin(thetaK)*sin(thetaK);
    hTheta[j] = GEPS / kSUM;
    printf(" NORM %lf  hTHETA %lf  ration %lf  >0.7 ?\n",norms[j], hTheta[j], fabs (norms[j] - hTheta[j])/hTheta[j]);
    if ( fabs (norms[j] - hTheta[j])/hTheta[j] >  0.7)
      invalid   = 1;
  }
  cross_product(&projV[0], &projV[3], proj );
  areaParallelogram = sqrt(proj[0] * proj[0] + proj[1] * proj[1] + proj[2] * proj[2]);
  if ( fabs( areaParallelogram - (hTheta[0] * hTheta[1] )/(hTheta[0] * hTheta[1] ) > 0.5) ) invalid = 1;
  /*    minmax[0] = 0.0; minmax[1] = 0.0;
    for ( j = 0 ; j < 2; ++j) {
      ...
      norm = 0.0;
      printf(" LINK %d  SID %d\n",link, sID);
      for ( i = 0 ; i < 3; ++i) dev[i] = qm ->xyzs[3*link + i ] - qm ->xyzs[3*sID + i ];
      norm  = sqrt(dev[0] * dev[0] + dev[1] * dev[1] + dev[2] * dev[2]);
      printf( " NORM %lf  \n",norm);
      if ( j ==0 ) {
        minmax[0] = norm; minmax[1] = norm;
      }
      if ( norm < qm -> qArea[6*q + 4]  ) {
        invalid   = 1;
        if (norm < minmax[0]) minmax[0] = norm;
      }
      if ( norm > qm -> qArea[6*q + 5]  ) {
        invalid   = 1;
        if (norm > minmax[1]) minmax[1] = norm;
      }
    }
    printf(" MIN MAX %lf  %lf  \n",minmax[0], minmax[1]);
    if ( fabs (minmax[0] - qm -> qArea[6*q + 3] ) < fabs (minmax[1] - qm -> qArea[6*q + 3] )  )
      minmax[0] = minmax[1];
  ...
  for ( i = 1; i < 4; ++i) {
    ...
    if ( (i != 2) && qm -> vType[v] == -1) {
      ...
      if( (movingVert == 1) && (angles[i] >= M_PI*0.95) && (angles[i] > alpha)) {
        alpha = angles[i];
        idx   = v;
        break;
      }
  ...
  *penalty = zerr * exp(zerr *scale/M_PI);
  printf(" PENALTY %lf  \n",*penalty);
  if ( invalid == 1) {
    //scale = zerr + exp(fabs(qm -> qArea[6 * q] - *quadArea) + fabs(qm -> qArea[6 * q + 3] - minmax[0]) );
    scale = zerr + exp(fabs(qm -> qArea[6 * q] - areaParallelogram) + fabs(norms[0] - hTheta[0]) +  fabs(norms[1] - hTheta[1]) ) ;
    if ( scale > *penalty) *penalty = scale;
  }
  //#ifdef DEBUG
  //if (OPTCOUNT%NOUT == 0 ) {
  printf("\n :::::::::: PENALTY FUNCTION AT VERTEX %d in QUAD %d : AREA = %lf  ANGLE AT VERT %lf-> VALIDAREA %d TOO SMALL TOO BIG = %d: AREA %lf   %lf LENGTH %lf %lf, %lf  %lf \n",
      qm ->quadIdx[4*(q) + (i + id0)%4], qID, *quadArea, angles[0], validArea, invalid, hTheta[0] * hTheta[1], areaParallelogram, norms[0],  hTheta[0], norms[1],  hTheta[1]);
...
if (PRINTAREA)
  ...
  *quadArea = area[0] + area[1];
  for ( k = 0 ; k < 4; ++k) {
    ...
    if ( (qm -> vType[qV[(k  + vPos )%4]] == -1) && (quadAngles[k] >= M_PI*0.95)) *validArea = -1;
...
EG_vertexSplit(quadMap *qm, int sID) {
  ...
  int id, idAux, stat, dist, validSplit, splitOK = 0;
  int  *swapPtr = NULL, *locIdx = NULL, *locAdj = NULL, *locVtype = NULL, **locVal = NULL;
...
if ( stat == EGADS_SUCCESS) splitOK = 1;
else {
#ifdef DEBUG
...
EG_splittingOperation(quadMap *qm, vStar *star, int poly[], int id0, int distanceToId0) {
  ...
  int q, newV, newQ, val, id, i, j, aux, stat, idAux, v0, dir[2], vertFromCollapse;
  ...
  if ( qm -> remQuads[0] > 0 ) {
    ...
    qm -> qArea    = EG_reall(qm -> qArea,    6*(qm -> nQ       + 1)*sizeof(double));
    ...
    if (qm->  quadIdx  == NULL ||qm ->  quadAdj == NULL ||qm ->  xyzs     == NULL ||
        qm -> uvs      == NULL ||qm ->  vType   == NULL ||qm ->  qArea == NULL ||
    ...
    newQ                          = qm -> nQ       + 1;
    for ( i = 0 ; i < 6; ++i)
      qm   -> qArea[6*qm -> nQ  + i] = qm -> qArea[6*(poly[0] -1 ) + i];
    qm   -> vType   [newV - 1]             = -1;
  ...
  qm -> uvs[2*(newV -1)    ] = qm -> uvs[2*v0    ] - 0.5 * dir[0];
  qm -> uvs[2*(newV -1) + 1] = qm -> uvs[2*v0 + 1] - 0.5 * dir[1];
...
getCurvatureAngle(quadMap *qm, int sID, const double *uvs, double *err ) {
  ...
  double dot[2*4], dMax, eAux,  curv[8], proj[4], normal[3], theta;
  ...
  for ( i = 0 ; i < 4; i++) {
    ...
    stat = EG_projectToTangentPlane(normal, &qm -> xyzs[3*vID], &qm -> xyzs[ 3*vs[i] ], proj );
  ...
  if ( qm -> setIndex ) {
    qm ->curvIdx[0] = iMax; qm ->curvIdx[1] = dMod;
  }
  else {
    // iMax = qm ->curvIdx[0]; dMod = qm ->curvIdx[1];
  }
  *err = 0.0;
  for ( i = 0 ; i < 4; ++i)
  {
    ...
    theta =   fabs(dot[2*iMod + dMod]);
    if      (      theta        < DEPS ) eAux = M_PI*0.5;
    else if ( fabs(theta - 1.0) < DEPS ) eAux = 0.0;
    else                                 eAux = fabs(acos(theta));
    // printf(" VERTEX %d = %d  dots % lf %lf  %lf \n", iMod, vs[iMod], dot[2*iMod], dot[2*iMod + 1], eAux);
    *err += eAux * eAux;
  }
  //printf(" ACC ERROR %lf\n", eAux);
  return EGADS_SUCCESS;
...
double NLOPT_objEqualAnglesTess(unsigned n, const double *uvs, double *grad, void *inQM)
{
  ...
  for ( i = 0 ; i < qm->nS; ++i) {
    ...
    if ( CURVATURE == 1 && peaks == 4) {
      // align with curvature lines
      //double fAux;
      //stat = getCurvatureAngle(qm, i, &uvs[2*i], &fAux);
