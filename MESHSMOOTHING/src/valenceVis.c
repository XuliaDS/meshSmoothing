...
static int EG_createQuadMap(bodyData *bodydata)
{
	...
	for ( f = 0 ; f < bodydata->nfaces; ++f) {
		...
		bodydata->qm[f] -> qArea    = (double*) EG_alloc(3*nquad  *sizeof(double));
		...
		for ( j = 0 ; j < nquad; ++j) {
			...
			bodydata->qm[f] -> qArea[3*j    ] =        surfArea;
			bodydata->qm[f] -> qArea[3*j + 1] = 0.75 * surfArea;
			bodydata->qm[f] -> qArea[3*j + 2] = 1.25 * surfArea;
		}
	}
...
computePenalty(quadMap *qm,  int starID, int qID, double *penalty, double *angles, double *quadArea) {
	...
	*penalty = zerr * exp(zerr *scale/M_PI);
	...
	if ( invalid == 1) {
		...
		scale = zerr + exp(fabs(qm -> qArea[3 * q] - areaParallelogram) + fabs(norms[0] - hTheta[0]) +  fabs(norms[1] - hTheta[1]) ) ;
...
EG_splittingOperation(quadMap *qm, vStar *star, int poly[], int id0, int distanceToId0) {
	...
	if ( qm -> remQuads[0] > 0 ) {
		...
		qm -> qArea    = EG_reall(qm -> qArea,    3*(qm -> nQ       + 1)*sizeof(double));
		...
		newQ                          = qm -> nQ       + 1;
		for ( i = 0 ; i < 3; ++i)
			qm   -> qArea[3*qm -> nQ  + i] = qm -> qArea[3*(poly[0] -1 ) + i];
		qm   -> vType   [newV - 1]             = -1;
