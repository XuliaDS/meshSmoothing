...
EG_edgeSwap(quadMap *qm, int sID) {
	int stat, i, j, k, aux, vL, movingVerts[6], nMove, v[4*2], vIrr[6], qID[2], link, swapped;
	...
	EG_free(uv0);
	EG_free(swapPtr);
	if ( stat != EGADS_SUCCESS ) printf(" STAT  SWAPPING OPERATION %d\n",stat);
	return stat;
}
...
static int EG_swapAndSplit(quadMap *qm, int sID ) {
	int    v1, v2, s, i, j, q, qq, sVAL, link, stat, validSwap, validSplit, qIDs[2], adj[2], id;
	int    *swapPtr = NULL, *locIdx = NULL, *locAdj = NULL, *locVtype = NULL, **locVal = NULL, vSwap[2*6];
	double *uv0 = NULL;
	vStar  *swapStar = NULL, *star = NULL;
	if ( qm -> vType[sID -1 ] == -2 || qm   -> valence[sID - 1][0] < 4 ) return EGADS_SUCCESS;
	stat = EG_buildStar(qm, &star, sID);
	if ( stat != EGADS_SUCCESS) return stat;
	...
	for ( s = 0 ; s < sVAL; ++s){
		v1      = star -> verts[2*s + 1];
		qIDs[0] = star -> quads[s]      ;
		if ( qm -> valence[v1 - 1][0] >= 5 && qm -> vType[v1 - 1] == -1) {
			// build a star around the high valence link in order to swap
			stat  = EG_buildStar(qm, &swapStar, v1);
			if ( stat != EGADS_SUCCESS) goto cleanup;
			...
			for ( q = 0 ; q < swapStar -> nQ; q++)
			{
				...
				validSwap = 0;
				for ( i = 0 ; i < 4; ++i) {
					if (qm -> quadAdj[4*(qIDs[1] - 1) + i] == qIDs[0]) {
				...
				if ( validSwap ) { // qID is in swapStar and not in star. lets see the valences
					...
					for ( i = 0 ; i < 4; i++) if ( qm -> quadIdx[4*(qIDs[0]-1) + i] == v1) id = i;
					for ( i = 0 ; i < 4; i++) {
						vSwap[2*i]     = qm -> quadIdx[4*(qIDs[0]-1) + (id + i)%2];
						vSwap[2*i + 1] = qm -> valence[vSwap[2*i] - 1][0];
					}
					// Add the remainnig two vertices from second quad
					for ( i = 0 ; i < 4; i++) if ( qm -> quadIdx[4*qIDs[1] + i] == v1) id = i;
					for ( i = 0 ; i < 2; i++) {
						vSwap[2*i + 4*2    ] = qm -> quadIdx[4*(qIDs[0] - 1) + (id + 2 + i)%2];
	...
	*totActivity = 0;
	for (i = 0 ; i < qm -> totVerts; ++i) {
		if ( (qm -> vType[i] > 0 && vertType > 0) ||  qm -> vType[i] == vertType  ) {
			//  printf(" CALL VERTEX OPERATOR WITH V %d OP %d  TOTAL ACTIVITY %d\n", i + 1, opType, *totActivity);
			stat = EG_vertexOperator(qm, i + 1, opType, &activity) ;
			*totActivity += activity;
			// printf(" ACTIVIY UPDATE %d\n", *totActivity);
			if ( stat != EGADS_SUCCESS) break;
			if ( (opType != TRYSWAPPING) && (activity == 1) ) {
				opType      = TRYSWAPPING;
				i           = -1;
...
{
	int     stat, auxID, activity, aux, totActivity,i, j, k, nOpt, optVec[MAXVALENCE];
...
int main(int argc, char *argv[])
{
	...
	int          nseg, ngp, atype, alen, quad, *segs, *senses, *tris, *heads,nledges, nh, nloops, *lsenses, *esenses;
	const int    *ints;
	float        arg, color[3];
	...
	for (ngp = sum = stat = iBody = 0; iBody < nbody; iBody++) {
		...
		if (argv[5]) {  //debugging-- no server call
			int f, meshImprovement;
			for (iBody = 0; iBody < nbody; iBody++) {
				for (f = 0; f < bodydata[iBody].nfaces; ++f) {
					if (bodydata[iBody].qm[f]-> uvs == NULL) continue;
					do  {
						stat = EG_fullMeshRegularization(bodydata[iBody].qm[f], 0, &meshImprovement, 0);
						printf(" MESH IMPROVEMENT %d\n",meshImprovement);
					}
					while(meshImprovement > 0);
					stat = EG_fullMeshRegularization(bodydata[iBody].qm[f], 0, &meshImprovement, 1);
					...
					if ( meshActivity ) {
						...
						for (iBody = 0; iBody < nbody; iBody++) {
							for (f = 0; f < bodydata[iBody].nfaces; ++f) {
								...
								if ( otype != TRYSWAPPING && region[vt] != -1) continue;
								if ( globalOpti ) {
									activity = 1 ;
								...
								if ( activity ) {
									...
									verts   = bodydata[iBody].qm[f]-> totVerts - bodydata[iBody].qm[f]-> remVerts[0];
									...
									for ( j = i = 0 ; i < bodydata[iBody].qm[f]-> totVerts; ++i) {
										if ( bodydata[iBody].qm[f]-> vType[i] == -2 ) continue;
										modxyzs[3*j]     = bodydata[iBody].qm[f]-> xyzs[3*i    ];
										modxyzs[3*j + 1] = bodydata[iBody].qm[f]-> xyzs[3*i + 1];
										modxyzs[3*j + 2] = bodydata[iBody].qm[f]-> xyzs[3*i + 2];
										stat = EG_evaluate(bodydata[iBody].qm[f]-> face, &bodydata[iBody].qm[f]->uvs[2*i], eval);
										if (stat!= EGADS_SUCCESS){
										...
										if ( bodydata[iBody].qm[f] -> face -> mtype == SREVERSE )
											cross_product(&eval[6], &eval[3], &normals[3*j]);
										else
											cross_product(&eval[3], &eval[6], &normals[3*j]);
										++j;
									}
									...
									if ( nquad < bodydata[iBody].qm[f]->nQ) printf("******* ****** ****   COLLAPSED EFFECT\n");
									ntri = 2*nquad;
									tris = (int *) malloc(3*ntri*sizeof(int));
